...
type UpdateMerchantChannel struct {
	ID          int     `json:"id"`
	DefaultRate float64 `json:"defaultRate"` // 默认费率
	SingleFee   float64 `json:"singleFee"`   // 单笔费用
	Weight      int     `json:"weight"`      // 权重值
...
type UpdateMerchantChannelStatus struct {
	ID     int    `json:"id"`
	Status string `json:"status"` // 1:开启 0:关闭
}
...
type MerchantChannelListRequest struct {
	...
	Type    *int8  `query:"type" form:"type"`
...
// RemoveMerchantChannel 删除新商户通道
type RemoveMerchantChannel struct {
	ID int `json:"id"`
}
