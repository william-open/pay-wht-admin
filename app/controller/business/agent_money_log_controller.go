...
type AgentMoneyLogController struct{}

// List 代理资金日志列表
func (*AgentMoneyLogController) List(ctx *gin.Context) {
...
// 代理资金日志详情
func (*AgentMoneyLogController) Detail(ctx *gin.Context) {
...
// Create 新增代理资金日志
func (*AgentMoneyLogController) Create(ctx *gin.Context) {
	...
	if Agent := (&service.AgentService{}).GetAgentByAgentName(param.Username); Agent.MId > 0 {
		response.NewError().SetMsg("新增代理资金日志" + param.Username + "失败，代理资金日志名称已存在").Json(ctx)
		return
	}
	u, _ := uuid.NewUUID()
	aesSecretKey, err := key.GenerateAESKey()
	if err != nil {
		response.NewError().SetMsg("新增代理资金日志失败，AES创建失败").Json(ctx)
		return
	}
	privatePEM, publicPEM, err := key.GenerateRSAKeys()
	if err != nil {
		response.NewError().SetMsg("新增代理资金日志失败,RSA创建失败").Json(ctx)
		return
	}
...
// Update 更新代理资金日志
func (*AgentMoneyLogController) Update(ctx *gin.Context) {
	...
	if Agent.MId > 0 && Agent.MId != param.MId {
		response.NewError().SetMsg("修改代理资金日志账号" + param.Username + "失败，代理资金日志账号已存在").Json(ctx)
		return
	}
	var editPassword = ""
	if param.Password == "" {
...
// Remove 删除代理资金日志
func (*AgentMoneyLogController) Remove(ctx *gin.Context) {
