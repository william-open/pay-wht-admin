...
func (s *TestingChannelService) CreateCollectOrder(param dto.TestCreatePayUpstreamProductRequest) (string, error) {
	...
	var requestParam dto.TestCreateReceiveOrderReq
	...
	// 请求下单服务
	var apiUrl = config.Data.TestDownMerchant.ReceiveApiUrl
...
// CreatePayoutOrder 代付订单
func (s *TestingChannelService) CreatePayoutOrder(param dto.TestCreatePayUpstreamProductRequest) (string, error) {
	...
	var requestParam dto.TestCreatePayoutOrderReq
	...
	requestParam.PayType = param.PayType
	requestParam.NotifyUrl = config.Data.TestDownMerchant.NotifyUrl
	requestParam.AccNo = param.AccNo
	requestParam.AccName = param.AccName
	requestParam.PayMethod = param.PayMethod
	requestParam.PayEmail = param.PayEmail
	requestParam.PayPhone = param.PayPhone
	requestParam.IdentityType = param.IdentityType
	requestParam.IdentityNum = param.IdentityNum
	requestParam.BankCode = param.BankCode
	requestParam.BankName = param.BankName
	pendingSignMap := utils.StructToMapString(requestParam)
	// 提取参数做签名（排除 Sign 字段）
	requestParam.Sign = utils.GenerateSign(pendingSignMap, merchant.ApiKey)
	// 请求下单服务
	var apiUrl = config.Data.TestDownMerchant.PayoutApiUrl
	log.Printf("请求GO API服务，下单地址: %v,请求参数: %+v", apiUrl, utils.StructToMapInterface(requestParam))
	body, err := curl.DefaultClient().Send(&curl.RequestParam{
		Header: map[string]interface{}{
			"Content-Type": "application/json",
